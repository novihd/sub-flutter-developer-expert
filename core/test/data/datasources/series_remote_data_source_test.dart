...
void main() {
  ...
  group('get Season Series', () {
    const tvId = 1;
    const seasonNumber = 1;

    final tSeasonList =
        (json.decode(readJson('dummy_data/season_detail.json'))['episodes']
                as List)
            .map((e) => SeasonDetailModel.fromJson(e))
            .toList();
    ...
    test('should return list of series when response code is 200 ', () async {
      ...
      // act
      final result = await dataSource.getSeriesSeason(
          tvId.toString(), seasonNumber.toString());
      // assert
      expect(result, tSeasonList);
    });
    ...
    test('should throw ServerException when response code is other than 200',
      ...
      // act
      final call =
          dataSource.getSeriesSeason(tvId.toString(), seasonNumber.toString());
      // assert
      expect(() => call, throwsA(isA<ServerException>()));
    });
  });
